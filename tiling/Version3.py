...
from tilingObjects.Version3Tile import Tile
from tiling.FieldFrame import FieldFrame
from tiling.Version3Field import Field
    ...
    def recursivePlacing(self):
        ...
        for position in tile.getPossiblePositions(): #determine position for the tile to be placed and place tile
            ...
            if self.field.placeTile(tile, x, y):
                self.frame.repaint(self.field)
                fillWhitespace = self.isolatedWhiteSpace() #returns true if no whitespace or filled
                if not fillWhitespace: #backtrack this tile if the whitespace cannot be filled
                    tile.removeFirstPosition()
                    self.field.removeTile(tile)
                    self.frame.repaint(self.field)
                self.steps += 1
                self.recursivePlacing()
                return
